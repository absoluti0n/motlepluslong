...
xml_file = r"C:\Users\julien.mayzou\Downloads\xmlwiktionary\frwiktionary-20231220-pages-articles-multistreamheaderedit5.xml"
                ...
                if is_french_word(title) and not is_first_letter_capitalized(title) and len(title)>4 and len(title)<11:
                    ...
                    if text_element is not None:
                        ...
                        if text and "{{langue|fr}}" in text:
                            if any(tag in text for tag in ["=== {{S|nom|fr", "=== {{S|adjectif|fr", "=== {{S|adverbe|fr", "=== {{S|pronom|fr"]):
                                liste_mots.append(title)
                            elif "=== {{S|verbe|fr" in text and any(subtag in text for subtag in ["ppr=oui}}", "ppfp=oui}}", "{{i|fr}}"]):
                                liste_mots.append(title)
                        itero += 1
            element.clear()
...
with open('outputxmltest5.csv', 'w', newline='', encoding='utf-8') as csvfile:
